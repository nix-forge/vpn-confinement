...
def normalized_policy(table):
    ...
    def clean(value):
        if isinstance(value, dict):
            return {
                k: (None if k == "counter" else clean(v))
...
def inspect(name, expected):
    ...
    routes = (
        json_command(prefix + [IP, "-j", "route", "show", "table", "all"])
        if exists
        else None
    )
    routes6 = (
        json_command(prefix + [IP, "-6", "-j", "route", "show", "table", "all"])
        if exists
        else None
    )
    table = (
        json_command(
            prefix + [NFT, "--json", "--stateless", "list", "table", "inet", "vpnc"]
        )
        if exists
        else None
    )
    chains = {
        ...
        for entry in (table or {}).get("nftables", [])
        if "chain" in entry
    }
    drop_chains = all(
        chains.get(c, {}).get("policy") == "drop"
    ...
    if not policy_match:
        issues.append(
            "installed firewall differs from the applied configuration or its snapshot is missing"
        )
    ...
    for route in (routes or []) + (routes6 or []):
        if route.get("dst") == "default" and route.get("dev") != expected["interface"]:
            issues.append(
                "default route does not use the configured WireGuard interface"
            )
    if not any(
        r.get("dev") == expected["interface"] and r.get("type", "unicast") == "unicast"
        for r in (routes or []) + (routes6 or [])
    ):
        issues.append("no usable route through the configured WireGuard interface")
    handshake_output = (
        command(prefix + [WG, "show", expected["interface"], "latest-handshakes"])
        if exists
        else None
    )
    ...
    if "peers" in expected and sorted(peer_hashes) != sorted(
        p["keyHash"] for p in expected["peers"]
    ):
        issues.append("installed WireGuard peers do not match configuration")
    if "peers" in expected:
        allowed_output = (
            command(prefix + [WG, "show", expected["interface"], "allowed-ips"])
            if exists
            else None
        )
        try:
            ...
            configured = {
                p["keyHash"]: networks(p["allowedIPs"]) for p in expected["peers"]
            }
            if allowed_output is None or installed != configured:
                issues.append(
                    "installed WireGuard allowed IPs do not match configuration"
                )
        except ValueError:
    ...
    if expected["endpointPinning"]:
        ...
        outer = (
            [] if birthplace in (None, "init") else [IP, "netns", "exec", birthplace]
        )
        pin_table = json_command(
            outer
            + [
                NFT,
                "--json",
        ...
        if not pin_match:
            issues.append(
                "endpoint-pinning policy differs from the applied configuration or its snapshot is missing"
            )
    services = {}
    inherited_sockets = set()
    for service in expected["services"]:
        unit = properties(service + ".service")
        inherited_sockets.update(
            s
            for field in ("TriggeredBy", "Sockets")
            for s in unit.get(field, "").split()
        ...
        if pid.isdigit() and int(pid) > 0:
            try:
                attached = (
                    os.stat(f"/proc/{pid}/ns/net").st_ino == ns_path.stat().st_ino
                )
                resolver_text = (
                    read_rooted(f"/proc/{pid}/root", "/etc/resolv.conf") or ""
                )
                resolvers = [
                    line.split()[1]
            ...
            if (
                expected["dns"]["mode"] == "strict"
                and resolvers != expected["dns"]["servers"]
            ):
                issues.append(
                    f"{service}: resolver configuration missing or unexpected"
                )
    ...
    socket_names = set(expected["sockets"]) | {
        s.removesuffix(".socket") for s in inherited_sockets
    }
    ...
    if expected.get("dns", {}).get("mode") == "compat" or expected.get("dns", {}).get(
        "allowHostResolverIPC"
    ):
    ...
    return {
        ...
        "defaultDropChainsPresent": drop_chains,
        "endpointPinningTablePresent": pin_installed,
        "policyMatchesConfiguration": policy_match,
        "endpointPolicyMatchesConfiguration": pin_match,
        "installedPeerCount": len(peer_hashes),
        "routesIPv4": routes,
        "routesIPv6": routes6,
        "services": services,
        "sockets": sockets,
        "handshakeAgesSeconds": ages,
...
def main():
    ...
    parser.add_argument(
        "namespace", nargs="?", help="configured namespace; default: all"
    )
    parser.add_argument(
    ...
    if os.geteuid() != 0:
        parser.exit(
            2, "Run with sudo to inspect namespaces and confined process mounts.\n"
        )
    configured = json.loads(Path(MANIFEST).read_text())["namespaces"]
    if args.namespace and args.namespace not in configured:
        parser.error(
            "unknown namespace; choose one configured in services.vpnConfinement"
        )
    ...
    if args.json:
        ...
        for item in report:
            ...
            print(
                f"  Firewall matches applied configuration: {item['policyMatchesConfiguration']}"
            )
            ...
            for name, service in item["services"].items():
                print(
                    f"  {name}: {service['unit'].get('ActiveState', 'unknown')}; process attached: {service['processAttached']}"
                )
        ...
        print(
            "Reachability was not probed. Idle handshake age is not a failure signal."
        )
        print(
            "These checks do not prove the absence of all alternate host communication paths."
        )
