...
let
  tableName =
    (import ../../modules/vpn-confinement/lib.nix { inherit lib; }).endpointTableName
      "vpnapps";
in
{
  ...
  testScript = ''
    ...
    machine.wait_until_succeeds("nft list table inet ${tableName} >/dev/null 2>&1")
    machine.succeed("nft list chain inet ${tableName} output | grep -Eq 'meta mark (0x[[:xdigit:]]+|[0-9]+) ip daddr 138\\.199\\.43\\.91 udp dport 51820 counter packets [0-9]+ bytes [0-9]+ accept'")

    machine.succeed("nft reset counters table inet ${tableName}")
    ...
    machine.wait_until_succeeds("nft list chain inet ${tableName} output | grep -Eq 'ip daddr 138\\.199\\.43\\.91 udp dport 51820 counter packets [1-9][0-9]* bytes [1-9][0-9]* accept'")
    ...
    machine.succeed("nft reset counters table inet ${tableName}")
    machine.succeed("ip netns exec vpnapps sh -c 'for _ in 1 2 3 4 5; do printf pin | nc -u -w 1 10.0.0.10 12345 || true; done'")
    machine.wait_until_succeeds("nft list chain inet ${tableName} output | grep -Eq 'udp counter packets [1-9][0-9]*( bytes [0-9]+)? drop'")
  '';
}
