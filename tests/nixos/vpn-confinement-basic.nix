{ pkgs, ... }:
{
  ...
  testScript = ''
    ...
    machine.succeed("systemctl show -p BindsTo --value netns-echo.service | grep -q 'vpn-confinement-netns@vpnapps.service'")
    machine.fail("test -e /run/vpn-confinement/vpnapps/resolv.conf")
    machine.fail("test -e /etc/netns/vpnapps/resolv.conf")
